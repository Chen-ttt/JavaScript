 ...
 * 打印一个实例化对象, 将看到
 *  1. 该对象自己的添加的属性
 *  2. 该对象的__proto__ : a. 通过类名添加的prototype属性
 *                        b. constructor
 *                        c. 上一级prototype
 */
...
console.log("phone1", p1)
console.log("phone1访问rom", p1.rom)
p1.call()

/**
 * 通过实例化的对象对prototype里的属性和方法进行增删改,都不行
 * 只能像上面一样, 用Phone.prototype
 */

delete p1.screen
console.log("删除原型中screen属性有效果吗?", Phone.prototype)
console.log("删除原型中screen属性有效果吗?", p1.__proto__)
p1.screen = '0'
console.log("修改原型中screen属性有效果吗?", Phone.prototype)
console.log("修改原型中screen属性有效果吗?", p1.__proto__)

/**
 * 原型一般写成对象形式
 *   但若用这种方式,则会直接覆盖prototype, 导致原有自带的属性会被覆盖掉,例如下面的constrcutor
 */
...
function People (name, age) {
  ...
  // 其实是
  // this = {
  //     __proto__: People.prototype // 使当前的实例化对象继承People类的原型中的属性
...
console.log(mary.constructor) // 实例化对象的constructor就是从类的原型中找到的
 ...
 *        而在使用了对象字面量对prototype重新赋值后, People.prototype就指向了新的object的地址; 但mary仍指向原来默认的prototype
 ...
 *   原因: 道理和情况1一样, 在重写ptototype之后, People.prototype(即构造函数中的原型)已经指向了新object的地址; 只有在重写后实例化的新对象(例如tong)才能指向新的prototype; 
 *        而在这之前实例化的旧对象(例如mary)仍指向之前的默认的老prototype,
 *        而直接赋值改变的是新prototype(因此能在constructor中的prototype看到改变为'333'); 而打印mary时, mary的prototype仍引用的是旧prototype, gender仍是‘Male’
 */
People.prototype.gender = '333' // 不能改
